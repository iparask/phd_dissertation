...
each stage of a pipelline follows a $G/G/s$ queue. A queue in a pipeline stage 
$S_{i}$ has arrival time $\alpha$, the distribution of the execution times from 
stage $S_{i-1}$, service time $\beta$, the distribution of the execution times 
of its tasks, and $r$ servicing resources. Initially, we will assume constant 
task size, static pipeline and known poisson distributions $\alpha$ and $\beta$ 
with event rate $\lambda$ experimentally calculated. This will allow us to 
create a model for the whole pipeline and predict the total time to execution. 
Next, we will relax the assumption and work task execution time distributions 
that are found experimentally. In addition, we will try to further relax our 
assumptions and introduce dynamic pipelines, and as a result dynamic workflows. 
% ------------------------------------------------------------------------------
