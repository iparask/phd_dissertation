...
Users usually offer this kind of information either through a function or a single number drawn by empirical data.
...
As a result, the actual runtime of the workflow $w'$ is $ w' = w \times (1-u')$.
...
As a result, we expect the sensitivity to workflow runtime estimation uncertainty for campaigns executing on homogeneous resources to be on similar across algorithms and we show only the results for heterogeneous workflows executing on heterogeneous resources.

Similar to sensitivity to resource dynamism, we expect sensitivity to workflow runtime estimantion uncertainty to decrease as the number of workflows increase and increase as the number of resources decrease.
...
Sensitivity to makespan runtime uncertainty has a wide range of values for HEFT and L2FF, but significantly smaller for GA-50 and random.
...
GA-50 plans show increased sensitivity for small number of workflows when the number of resources is constant, above 10~\%, and quickly drop close to 0~\%.

Generally, HEFT shows higher sensitivity than GA-50 and L2FF above 128 workflows/4 resources and when the number of resources change and the campaign size is 1024 workflows.
...
This in turn makes HEFT more sensitive to changes and for some resources there are less chances to hide the effect of a single workflow that needs more time than the expected.
...
We believe that unexpected behavior from HEFT is a consequence of prioritizing workflows and placing a workflow on the resource that will finish it earlier.
...
This in conjunction with experiment 2 allows to conclude that it is independent from the type of uncertainty.
...
Based on our investigation, we conclude that there are three factors that affect the performance and sensitivity of planning algorithms.
The first factor is whether the algorithm uses an expectation of when a resource will be available.
HEFT and the genetic algorithm use an expectation of resource availability and show better performance on homogeneous and heterogenous resources than L2FF and random.
...
From a user's perspective, without loss of generality, an algorithms which is deterministic, uses an expectation of resource availability and creates a priority list, is the right selection for planning the execution of a campaign on HPC resources before actually executing the campaign.
The plans using HEFT always provide better makespan even with 25~\% increase due to workflow runtime uncertainy.
