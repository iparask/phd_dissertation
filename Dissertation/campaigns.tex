...
\begin{equation}
F_{i} = 1 - \sqrt{(\sum_{j=1}^{N_{R}} \frac{w_{j}}{r_{j}} - IF)^2}
\label{eq:fitness}
\end{equation}
where $N_{R}$ is the number of resources and $IF$ is the ideal makespan.
The ideal makespan is equal to
\begin{equation}
IF = \frac{\sum_{i=1}^{N_{C}}T_{w_{i}}}{\sum_{j=1}^{N_{R}}r_{j}}
