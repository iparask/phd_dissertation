...
depending on large scale experiments~\cite{atlas} benefit from
...
Quantum chemistry campaigns~\cite{smith2020molssi}, for example, usually 
execute hundreds of workflows analyzing molecules for months and on different 
platforms, such as workstations, campus and high performance computing (HPC) 
resources~\cite{smith2020molssi}, and store the results in a database. 
Ecological sciences that utilize very high resolution satellite 
imagery~\cite{goncalves2020sealnet}, analyze terabytes of data from different 
calendar years to derive time series of ecological changes. These sciences 
execute workflows on HPC with simple or complex 
...
\begin{inparaenum}[(1)]
    ...
    \item use of multiple HPC computational resources;
...
abstraction is successfully used to execute data analytics in different 
domains~\cite{hellerstein2012science}. Utilizing the MapReduce abstraction to 
scalably analyze data has the potential to increase resource utilization and 
reduce time of completion of data analytics on HPC. However, MapReduce 
frameworks, such as Hadoop~\cite{hadoop} and Spark~\cite{zaharia2010spark}, 
mainly support cloud resources and not HPC. There is therefore a need for 
computing environments that support MapReduce frameworks on HPC resources while 
allowing their interoperability with HPC workflows. To the best of our 
