...
% For example, one of the use cases described in Ref.~\cite{borodin2015big}
% analyzes thousands of different datasets with workflows running for months
...
requirements:
% . Some of the most important common characteristics of these
% campaigns are:
\begin{inparaenum}[(1)]
...
effectively support the execution of data-driven computational campaigns on HPC
resources. This has the potential to increase overall resource utilization
...
and efficient execution of data-driven computational campaigns on HPC resources.
It is important to note that our solutions and results are agnostic towards the
scientific domain of the computational campaigns.

The effective and efficient execution of a data-driven computational campaign
poses several engineering challenges.
...
Utilizing the MapReduce abstraction to scalably analyze data has the potential
to increase resource utilization and reduce the time of completion of data
analytics on HPC platforms. However, frameworks that implement MapReduce, such
as Hadoop~\cite{hadoop} and Spark~\cite{zaharia2010spark}, mainly support cloud
and not HPC resources. There is therefore a need for computing environments that
support frameworks like Hadopp and Spark on HPC resources while allowing their
interoperability with HPC workflows \mtnote{I have no idea what and HPC workflow
is and what interoperability in this context means. Careful, you are introducing
a ton of vocabulary and assuming a ton of knowledge from the reader.}. To the
best of our knowledge, there does not exist a solution that provides the
integrated capabilities of MapReduce and HPC resources.

Data analytics frameworks exploit task-parallelism to execute data analytics
...
Data analysis workflows may require to execute data-intensive tasks,
compute-intensive tasks or both. Frameworks that support the execution of data
analysis workflows are designed to effectively and efficiently support a
specific type of workflow task.

\mtnote{here there is another jump. I rewrote the beginning of the paragraph,
    does it help? I stop here and I let you go through another iteration.}

For example, Spark, may not be the most
...
design that can efficiently execute data-driven compute-intensive workflows is
challenging. As a result, there is a need to understand which design is best
suited to execute data-driven compute-intensive workflows.

Currently, campaign managers make assumptions about the resources and the
...
significantly changing their implementations. In addition, based on the
assumptions about the middleware they cannot support workflows that do not have
similar characteristics and are developed using different frameworks. As a
result, there is a need for a campaign manager that supports the scalable
execution of computational campaigns on HPC resources and is agnostic of the
middleware used and scientific domain.
...
HPC resources. While computational campaigns can utilize multiple HPC resources
concurrently, the effectiveness of a plan is not guaranteed. Actual HPC
resources performance is affected by several factors and can change
significantly during the execution of a campaign. It is important, then, this
execution plan to efficiently map the workflows of the campaign, so as to
increase resource utilization and achieve the objective of the campaign. As
