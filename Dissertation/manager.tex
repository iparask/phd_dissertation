...
Second, it allows us to quickly implement and test whether the selected design supports the requirements of the supported use cases.
...
This way, it is agnostic of the system used to manage the execution of the campaign workflows allowing us to utilize all the capabilities presented in the previous chapters.
In addition, the campaign manager makes no assumptions about the resources on which the campaign workflows will be mapped and executed.
...
The space of data analysis computational campaigns can be vast with different scientific objectives, types and number of workflows and resources they utilize.
As a result, we use three real use cases with data analysis workflows to derive the requirements of the campaign manager prototype.
...
Quantum chemistry campaigns execute workflows that execute chemical computations on molecules to calculate their properties.
Computational scientist through MolSSI~\cite{smith2020molssi} execute thousands of workflows, with up to 1000 executing at any given point in time, to a number of different resources.
...
Earth science campaign analyze imagery acquired via sensors from different calendar years~\cite{paraskevakos2019workflow} to create time series of ecological events.
Their campaigns requires the execution of one workflow per calendar year of data.
The workflows analyze a set of satellite images and their execution time varies from hours to a couple of days.
In this use case, users have access to the resources where their imagery is stored.
While analyzing the data based on the year they represent, a specific order is not necessary as long as all datasets are analyzed.
...
The architecture of the campaign manager is described in \S~\ref{ssec:cm_arch}.
...
The planner derives an execution plan based on a planning algorithm and calculates the makespan for a campaign and a set of resources.
It receives information about available resources and the workflows of the campaign from the bookkeeper component.
The planner does not adhere to a specific planning algorithm allowing different planning to be added and used based on the campaign's requirements and objective.
...
The workflows state model defines six states: 1) \textit{new}, 2) \textit{executing}, 3) \textit{done}, 5) \textit{canceled} and 6) \textit{failed}.
...
After a plan is produced, \textit{work} calls \textit{\_verify\_objective} to verify if the objective of the campaign can be achieved and starts submitting workflows to the enactor.
...
In addition, it executes the 128 workflows on less than 2 minutes, when in reality executing a campaign can be up to months.
