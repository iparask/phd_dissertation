...
capabilities or runtime systems. QCFractal \mtnote{reference?} is built to be
...
campaign manager has to support use cases from different scientific domains,
...
implement and test the campaign manager without necessarily considering any
...
allows us to tune parts of the campaign manager with minimal engineering cost.

The campaign manager prototype is domain agnostic as its design does not depend
...
The chapter is organized as follows: In Section~\ref{sec:cm_req}, we discuss the
supported use cases and the campaign manager requirements.
Section~\ref{sec:cm_des} describes the building blocks approach and the design
of the campaign manager. In Section~\ref{sec:cm_impl} we describe the
implementation of the campaign manager prototype and we characterize its
...
campaign manager prototype. The first use case supports quantum chemistry
...
of the campaign manager, such as executing individual workflows and monitoring
the campaign.
...
classes to implement the campaign manager prototype. The Bookkeeper class
...
time, as shown in Figure~\ref{fig:cm_char}. We observed that the execution time of the simulation increases when more than 64
...
components of the campaign manager, a Bookkeeper, a Planner and an Enactor.
