...
Spark and Dask, we investigate RADICAL-Pilot~\cite{merzky2019using}. We utilize
...
The chapter is organized as follows: Section~\ref{sec:tp_related_work} discusses
several approaches to support scalable MD trajectory data analytics.
Section~\ref{sec:md_use_cases} describes the MD analysis algorithms investigated
...
Section~\ref{sec:frameworks} describes RADICAL-Pilot, Spark and  Dask, the three
frameworks used for our performance comparison and evaluation.
Section~\ref{sec:impl_exp} provides a description of the implementation of the
MD algorithms on top of those three frameworks, as well as a performance
evaluation and a discussion of our findings. In
Section~\ref{sec:task_sel_model}, we provide a conceptual analysis that allows
application developers to select a framework according to their requirements.
Finally, Section~\ref{sec:tp_concl} closes this chapter with our conclusions.
...
%size. Through RADICAL-Pilot, Spark and Dask, we avoided recompiling every time
...
and RADICAL-Pilot support the execution of bag of tasks, with RADICAL-Pilot and
...
MapReduce and implementing LF is relatively straightforward. RADICAL-Pilot, on
...
RADICAL-Pilot~\cite{merzky2019using} for MD data analytics.
...
RADICAL-Pilot also offers a Python-only programming API, supporting task-level
parallelism on HPC resources. In this way, RADICAL-Pilot adds parallelization
...
As discussed in~\S\ref{ch:pilot-data-hadoop}, RADICAL-Pilot allows concurrent
task execution on HPC resources. The user defines a set of Compute-Units (CU)
which are submitted to RADICAL-Pilot. RADICAL-Pilot schedules these CUs for
...
%RADICAL-Pilot's learning curve can be quite steep at the beginning, at least
%until the user becomes familiar with the concept and usability of Pilots and
%CUs. Once the user is comfortable with RADICAL-Pilot's API, she can easily
...
%\subsubsection*{RADICAL-Pilot} RADICAL-Pilot~\cite{merzky2019using} is a Pilot
%system that implements the pilot paradigm as outlined in
%Ref.~\cite{turilli2018comprehensive}. RADICAL-Pilot (RP) is implemented in
...
%RADICAL-Pilot allows concurrent task execution on HPC resources. The user
%defines a set of Compute~-Units (CU)~- the abstraction that defines a task
%along with its dependencies - which are submitted to RADICAL-Pilot.
...
RADICAL-Pilot with respect to the abstractions and runtime systems provided to
create and execute parallel data applications.

\subsubsection*{API and Abstractions}

RADICAL-Pilot provides a low-level API for executing tasks onto resources. While
...
High-level tools for RADICAL-Pilot, such as Ensemble
Toolkit~\cite{balasubramanian2018harnessing}, are designed for workflows
involving compute-intensive tasks. Spark and Dask already offer a set of
high-level data-oriented abstractions, such as Dataframes. \mtnote{I am not sure
...
stage-oriented scheduler introduces. RADICAL-Pilot does not provide a DAG-based
API and requires the user to manage the execution order and synchronization
among tasks at application level instead of runtime level (i.e., RADICAL-Pilot).
...
RADICAL-Pilot, Spark and Dask and measure the execution time on a single node.

For RADICAL-Pilot, all tasks were submitted simultaneously. RADICAL-Pilot's
...
tasks, followed by Spark and RADICAL-Pilot. Dask and Spark quickly reach their
maximum throughput, which is sustained as the number of tasks increased.
RADICAL-Pilot showed the worst throughput and scalability, mainly due to some
architectural limitations. Specifically, RADICAL-Pilot relies on a MongoDB
database to communicate between Client and Agent, as well as several components
that allow RADICAL-Pilot to move data and introduce delays in the execution of
the tasks. As a result, we were not able to scale RADICAL-Pilot to 32k or more
tasks.
...
Spark's throughput is an order of magnitude lower than Dask's. RADICAL-Pilot's
...
when implemented with RADICAL-Pilot, Spark and Dask. We compare the performance
...
carried using RADICAL-Pilot and Pilot-Spark (as discussed
on~\S\ref{ch:pilot-data-hadoop}). We utilize a set of custom scripts to start
the Dask cluster. We used RADICAL-Pilot 0.46.3, Spark 2.2.0, Dask 0.14.1 and
Distributed 1.16.3.
...
For RADICAL-Pilot, we define a Compute-Unit for each task and execute them using
...
MPI4py, RADICAL-Pilot, Spark and Dask have similar performance when used to
...
enough to impact the frameworks' speedup. Note that RADICAL-Pilot's large
...
frameworks. Both RADICAL-Pilot and Dask are native Python frameworks, making the
...
We developed five approaches to implement the LF algorithm using RADICAL-Pilot,
...
Spark, Dask and MPI4py. RADICAL-Pilot's performance is illustrated in
...
\texttt{map} task computes the pairwise distance on its partition. \mtnote{its
...
comparable to MPI4py for the $262k$, and $524k$ datasets. It \mtnote{I am not
able to understand to what this 'it' refers to.} also shows better performance
...
approach~\ref{en:2} for RADICAL-Pilot. As expected, this approach overcomes the
...
at least a factor of two \mtnote{compared to another middlewre or compared to
approach 1?}. However, we were not able to scale this implementation to the 4M
dataset due to the memory requirements of \texttt{cdist}. For RADICAL-Pilot, we
observed significant task management overheads (see also
section~\ref{sec:framework_eval}). This is a limitation of RADICAL-Pilot with
respect to managing large numbers of tasks, particularly visible when running on
a single node with 32 cores. As more than 64 cores become available, performance
\mtnote{of RADICAL-Pilot or for all the middleware?} improves dramatically.
...
using Spark, Dask and RADICAL-Pilot, as well as MPI4py. Implementation aspects,
...
Dask, clearly outperforms RADICAL-Pilot
...
allows for faster prototyping compared to RADICAL-Pilot and Spark.
RADICAL-Pilot's learning curve is more steep, but is more versatile than Dask
...
RADICAL-Pilot is well suited for HPC applications, e.g., ensembles (up to $50k$
...
data-intensive workloads. The file staging implementation of RADICAL-Pilot is
...
on the same resource makes RADICAL-Pilot particularly suitable when different
...
assessed the architectures of RADICAL-Pilot, Spark and Dask. We provided a
...
python frameworks. RADICAL-Pilot does not provide a MapReduce API, but is well
